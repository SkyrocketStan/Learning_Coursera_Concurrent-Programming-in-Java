...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
  ...
  public void computeBoruvka(
      final Queue<ParComponent> nodesLoaded, final SolutionToBoruvka<ParComponent> solution) {

    ParComponent current;

    while ((current = nodesLoaded.poll()) != null) {
      if (current.isDead) {
        continue;
      }
      ...
      final Edge<ParComponent> edge = current.getMinEdge();
      if (edge == null) {
        current.lock.unlock();
      ...
      final ParComponent other = edge.getOther(current);
      ...
      other.isDead = true;
      current.merge(other, edge.weight());
      other.lock.unlock();
      current.lock.unlock();
  ...
  public static final class ParComponent extends Component<ParComponent> {
    ...
    /**
     * The weight this component accounts for. A component gains weight when it is merged with
     * another component across an edge with a certain weight.
     */
